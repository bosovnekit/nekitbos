...
local function createDefaultConfig()
    ...
    if file then
        file:write("; Helper Lovli Configuration File\n")
        file:write("; Автоматически создано при первом запуске\n\n")
        file:write("[script]\n")
        file:write("scriptColor = " .. DEFAULT_CONFIG.script.scriptColor .. "\n")
        file:write("activationCommand = " .. DEFAULT_CONFIG.script.activationCommand .. "\n\n")
        
        file:write("[cef]\n")
        file:write("timeout = " .. DEFAULT_CONFIG.cef.timeout .. "\n")
        file:write("houseEnabled = " .. tostring(DEFAULT_CONFIG.cef.houseEnabled) .. "\n")
        file:write("businessEnabled = " .. tostring(DEFAULT_CONFIG.cef.businessEnabled) .. "\n\n")
        
        file:write("[trening]\n")
        file:write("enabled = " .. tostring(DEFAULT_CONFIG.trening.enabled) .. "\n")
        file:write("key = " .. DEFAULT_CONFIG.trening.key .. "\n")
        file:write("command = " .. DEFAULT_CONFIG.trening.command .. "\n\n")
        
        file:write("[marker]\n")
        file:write("defaultMode = " .. DEFAULT_CONFIG.marker.defaultMode .. "\n")
        file:write("enabled = " .. tostring(DEFAULT_CONFIG.marker.enabled) .. "\n\n")
        
        file:write("[timer]\n")
        file:write("enabled = " .. tostring(DEFAULT_CONFIG.timer.enabled) .. "\n")
        
        file:close()
        print("Helper Lovli: Создан файл конфигурации по умолчанию")
    end
end
...
local function loadConfig()
    ...
    -- Если файл не существует, создаём его с настройками по умолчанию
    if not file_exists(CONFIG_PATH) then
        print("Helper Lovli: Файл конфига не найден, создаём новый")
        createDefaultConfig()
    ...
    if success and loaded then
        ...
        if next(loaded) == nil then
            print("Helper Lovli: Конфиг пуст, создаём новый")
            createDefaultConfig()
            return DEFAULT_CONFIG
        end
        ...
        print("Helper Lovli: Ошибка загрузки конфига, создаём новый")
        createDefaultConfig()
        return DEFAULT_CONFIG
    end
end
