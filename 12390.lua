...
if not imgui or not samp_check or not effil_check or not monet_check then
    function main()
        ...
        for k, v in pairs(libs) do
            if not v then
                sampAddChatMessage('« Helper Lovli » {FFFFFF}Ó Âàñ îòñóòñòâóåò áèáëèîòåêà {7172ee}' .. k .. '{FFFFFF}. Áåç íå¸ ñêðèïò {7172ee}íå áóäåò {FFFFFF}ðàáîòàòü!', 0x7172ee)
                table.insert(libs_no_found, k)
            end
        end
        sampShowDialog(18364, '{7172ee}Helper Lovli', string.format('{FFFFFF}Â Âàøåé ñáîðêå {7172ee}íåòó íåîáõîäèìûõ áèáëèîòåê{FFFFFF} äëÿ ðàáîòû ñêðèïòà.\nÁåç íèõ îí {7172ee}íå áóäåò{FFFFFF} ðàáîòàòü!\n\nÁèáëèîòåêè, êîòîðûå Âàì íóæíû:\n{FFFFFF}- {7172ee}%s\n\n{FFFFFF}Âñå áèáëèîòåêè ìîæíî ñêà÷àòü â òåìå íà BlastHack: {7172ee}https://www.blast.hk/threads/190033\n{FFFFFF}Â ýòîé æå òåìå Âû {7172ee}íàéäåòå èíñòðóêöèþ {FFFFFF}äëÿ èõ óñòàíîâêè.', table.concat(libs_no_found, '\n{FFFFFF}- {7172ee}')), 'Ïðèíÿòü', '', 0)
        thisScript():unload()
    end
    return
end
...
-- Çàãðóæàåì êîíôèã ñ çàùèòîé
local function loadConfig()
    -- Åñëè ôàéë íå ñóùåñòâóåò, âîçâðàùàåì äåôîëòíûé
    if not file_exists(CONFIG_PATH) then
        print("Helper Lovli: Ôàéë êîíôèãà íå íàéäåí, èñïîëüçóåòñÿ äåôîëòíûé")
        return DEFAULT_CONFIG
    end
    
    -- Ïûòàåìñÿ çàãðóçèòü êîíôèã
    local success, loaded = pcall(inicfg.load, DEFAULT_CONFIG, CONFIG_PATH)
    if success and loaded then
        -- Ïðîâåðÿåì, íå ïóñòîé ëè êîíôèã
        if next(loaded) == nil then
            print("Helper Lovli: Êîíôèã ïóñò, èñïîëüçóåòñÿ äåôîëòíûé")
            return DEFAULT_CONFIG
        end
        print("Helper Lovli: Êîíôèã óñïåøíî çàãðóæåí")
        return loaded
    else
        print("Helper Lovli: Îøèáêà çàãðóçêè êîíôèãà, èñïîëüçóåòñÿ äåôîëòíûé")
        return DEFAULT_CONFIG
    end
end
...
-- Íàñòðîéêè runtime ñ çàùèòîé îò nil
local settings = {
...
function saveConfig()
    ...
    -- Óáåæäàåìñÿ, ÷òî âñå ñåêöèè ñóùåñòâóþò
    ...
    if not success then
        print("Helper Lovli: Îøèáêà ñîõðàíåíèÿ êîíôèãà - " .. tostring(err))
    else
        print("Helper Lovli: Êîíôèã ñîõðàíåí")
    end
end
...
function showCaptcha()
    ...
    sampShowDialog(8813, '{F89168}Òðåíèðîâêà êàï÷è', '{FFFFFF}Ââåäèòå {C6FB4A}5{FFFFFF} ñèìâîëîâ, êîòîðûå\nâèäíî íà {C6FB4A}âàøåì{FFFFFF} ýêðàíå.', 'Ïðèíÿòü', 'Îòìåíà', 1)
    captime = os.clock()
    trening_active = true
...
function getHouseName(id)
    ...
    elseif id == 541 then return "Êðóòàÿ Îñîáà"
    elseif id == 495 or id == 124 or id == 491 or id == 307 or id == 413 or id == 624 or id == 255 then return "Ãðèí-Òàóí"
    elseif id >= 850 and id <= 1300 then return "Ñàí-Ôèåðî"
    end
    return nil
end

function getBusinessName(id)
    if id >= 360 and id <= 397 then return "Ïàðêî Çàâîä"
    elseif id >= 398 and id <= 408 then return "Ïàðêî Àýðî Ëñ"
    elseif id >= 409 and id <= 410 then return "Ïàðêî Äá ËÑ"
    elseif id >= 443 and id <= 450 then return "Âîäíûå ïàðêîâêè"
    end
    return nil
end

-- ============= ÒÅÑÒÎÂÀß ÔÓÍÊÖÈß ÄËß ÏÐÎÂÅÐÊÈ CEF =============

function testCEF()
    show_arz_notify('info', 'Òåñò CEF', 'Åñëè âû ýòî âèäèòå - CEF ðàáîòàåò!', 5000)
    sms('Îòïðàâëåí òåñòîâûé CEF íîòèôàé')
end

-- Ðåãèñòðèðóåì òåñòîâóþ êîìàíäó
    ...
    if msg == 0x100 and waiting_for_key then
        ...
        if key then
            if waiting_for_key_type == "new" then
                ...
                saveConfig()
                sms('Áèíäåð äîáàâëåí: ' .. key .. ' | ' .. tostring(CMD_NAMES[new_cmd_idx[0] + 1]))
...
function onShowDialog(dlgId, style, title, button1, button2, text)
    ...
    -- Ïðîâåðÿåì äèàëîã ïîêóïêè èìóùåñòâà
    if title and (title:find('Ðàçðåøåíèå íà ïîêóïêó èìóùåñòâà') or title:find('Ðàçðåøåíèå íà ïîêóïêó')) then
...
-- Ðåãèñòðèðóåì îáðàáîò÷èê äèàëîãîâ ÷åðåç samp.events
if samp_check then
...
function main()
    ...
    sampRegisterChatCommand('ontr', function() trening_enabled[0] = not trening_enabled[0]; saveConfig(); sms('Òðåíèíã ' .. (trening_enabled[0] and '{mc}âêëþ÷åí' or 'âûêëþ÷åí')) end)
    sampRegisterChatCommand('captcha', function() trening_enabled[0] = not trening_enabled[0]; saveConfig(); sms('Òðåíèíã ' .. (trening_enabled[0] and '{mc}âêëþ÷åí' or 'âûêëþ÷åí')) end)

    sms('{mc}Helper Lovli{ffffff} çàãðóæåí! Àâòîðû: {mc}bossov{ffffff} & {mc}slardar')
    sms('Àêòèâàöèÿ ìåíþ: {mc}/' .. settings.activationCommand)
    sms('Çàãðóæåíî áèíäåðîâ: {mc}' .. #binders)
    ...
    imgui.OnFrame(
        ...
        function()
            if window[0] then
                ...
                if menu[0] == 1 then
                    imgui.PushFont(fonts[18])
                    imgui.FText(u8"Öâåò ñêðèïòà:", 18)
                    imgui.SameLine()
                    ...
                    imgui.PopItemWidth()
                    imgui.SameLine()
                    if imgui.ActiveButton(u8" Ïðèìåíèòü", imgui.ImVec2(100, 25)) then
                    ...
                    if imgui.ActiveButton(u8" Ñîõðàíèòü âñå íàñòðîéêè", imgui.ImVec2(250, 30)) then
                        saveConfig()
                        sms('Íàñòðîéêè ñîõðàíåíû')
                    end
                    imgui.PopFont()
                ...
                elseif menu[0] == 2 then
                    ...
                    if imgui.ActiveButton(u8" Äîáàâèòü", imgui.ImVec2(140, 25)) then
                        ...
                        sms('Íàæìèòå êëàâèøó äëÿ íîâîãî áèíäåðà...')
                    end
                ...
                elseif menu[0] == 3 then
                    imgui.PushFont(fonts[18])
                    imgui.SetCursorPosX(100)
                    imgui.FText(u8"Âêëþ÷èòü òðåíèíã:", 18)
                    imgui.SameLine(300)
                    ...
                    imgui.Dummy(imgui.ImVec2(0, 10))
                    imgui.SetCursorPosX(100)
                    if imgui.ActiveButton(u8" Îòêðûòü êàï÷ó ñåé÷àñ", imgui.ImVec2(250, 30)) then
                        showCaptcha()
                    end
                    imgui.SameLine()
                ...
                elseif menu[0] == 4 then
                    imgui.PushFont(fonts[18])
                    imgui.SetCursorPosX(100)
                    imgui.FText(u8"Âêëþ÷èòü ìàðêåð:", 18)
                    imgui.SameLine(300)
                    if imgui.Checkbox('##marker_enabled', marker_enabled) then saveConfig() end
                    imgui.Dummy(imgui.ImVec2(0, 10))
                    imgui.SetCursorPosX(100)
                    imgui.FText(u8"Ðåæèì ïî óìîë÷àíèþ:", 18)
                    imgui.SameLine(300)
                    local mode_names = { u8"Òðåéñåð", u8"Ìàðêåð" }
                    ...
                    imgui.FText(u8"{ButtonActive}/marker{Text} - ïîñòàâèòü ìàðêåð", 18)
                    imgui.SetCursorPosX(120)
                    imgui.FText(u8"{ButtonActive}/delmarker{Text} - óäàëèòü ìàðêåð", 18)
                    imgui.PopFont()

                elseif menu[0] == 5 then
                    imgui.PushFont(fonts[18])
                    imgui.SetCursorPosX(100)
                    imgui.FText(u8"Âêëþ÷èòü òàéìåð:", 18)
                    imgui.SameLine(300)
                    if imgui.Checkbox('##timer_enabled', timer_enabled) then saveConfig() end
                    imgui.Dummy(imgui.ImVec2(0, 20))
                    imgui.SetCursorPosX(100)
                    imgui.FText(u8"Òàéìåð ïîêàçûâàåò âðåìÿ,", 18)
                    imgui.SetCursorPosX(100)
                    imgui.FText(u8"çà êîòîðîå âû ââåëè êàï÷ó", 18)
                    imgui.PopFont()

                elseif menu[0] == 6 then
                    imgui.PushFont(fonts[18])
                    imgui.SetCursorPosX(100)
                    imgui.FText(u8"Âðåìÿ ïîêàçà óâåäîìëåíèé:", 18)
                    imgui.SameLine(300)
                    imgui.PushItemWidth(100)
                ...
                elseif menu[0] == 7 then
                    ...
                    imgui.SetCursorPosX(150)
                    imgui.FText(u8"Âåðñèÿ: 2.1.0", 18)
                    imgui.SetCursorPosX(150)
                    imgui.FText(u8"Àâòîðû: idea - bossov, realization - slardar", 18)
                    imgui.Dummy(imgui.ImVec2(0, 20))
                    imgui.SetCursorPosX(150)
                    imgui.FText(u8"Êîìàíäû:", 18)
                    imgui.SetCursorPosX(170)
                    imgui.FText(u8"{ButtonActive}/" .. settings.activationCommand .. u8"{Text} - îòêðûòü ìåíþ", 18)
                    imgui.SetCursorPosX(170)
                    ...
                    imgui.FText(u8"{ButtonActive}/delmarker{Text} - óäàëèòü ìàðêåð", 18)
                    imgui.SetCursorPosX(170)
                    imgui.FText(u8"{ButtonActive}/ontr /captcha{Text} - òðåíèíã", 18)
                    imgui.PopFont()
                end
                ...
                imgui.Separator()
                imgui.BeginGroup()
                if imgui.ActiveButton(u8" Ñîõðàíèòü", imgui.ImVec2(130, 28)) then
                    saveConfig()
                    sms('Íàñòðîéêè ñîõðàíåíû')
                end
                imgui.SameLine()
                if imgui.Button(u8" Çàêðûòü", imgui.ImVec2(130, 28)) then
                    window[0] = false
                    imgui.Process = false
                end
                imgui.EndGroup()
            ...
            if show_cmd_popup[0] then
                imgui.OpenPopup(u8"Âûáîð êîìàíäû")
                imgui.SetNextWindowSize(imgui.ImVec2(300, 350), imgui.Cond.Always)
                if imgui.BeginPopupModal(u8"Âûáîð êîìàíäû", show_cmd_popup, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse) then
                    imgui.FText(u8"Âûáåðèòå êîìàíäó:", 18)
                    imgui.Separator()
    ...
    while true do
            ...
            if result then
                if button == 1 then
                    if input == captcha..'0' then
                        sms('{1DFF00}Âåðíî! [' .. string.format("%.3f", os.clock() - captime) .. ' ñåê]')
                    else
                        sms('{FF0000}Íåâåðíî! [' .. string.format("%.3f", os.clock() - captime) .. ' ñåê]')
                    end
                end
                removeTextdraws()
                trening_active = false
        ...
        if timer_enabled[0] then
            if sampIsDialogActive() and sampGetDialogCaption():find('Ïðîâåðêà íà ðîáîòà') then
                ttime = os.clock()
